...
export const buildPage = (page: Page, widgets: KeyedObject<Widget>, layouts: KeyedObject<Layout>, pageNode: TreeNode) => {
  const makeElement = (node: TreeNode): React.ReactNode => {
    const element = (() => {
      if (node.type === 'widget')
        return buildWidget(widgets[node.id]);
      if (node.type === 'layout')
        return buildLayout(layouts[node.id]);
      throw Error();
    })();
